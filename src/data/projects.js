...
export const projects = [
...

  {
    ...
    highlights: [
      "Star schema of seven dimensions and two facts, 61,459 encounters and 38,094 conditions, keyed on the natural identifiers the feed supplies rather than hashed surrogates. The patient and date dimensions are conformed, shared across both facts, and the condition fact joins the date dimension twice, once for the start of the condition and once for its end",
      "207 tests: 133 not_null, 21 unique, 19 relationships, 15 accepted_values and 19 singular assertions. Every foreign key in the project resolves with zero orphans",
    ...
    caseStudy: {
      sections: [
        ...
        },
        {
          ...
          body: [
            "Seven dimensions and two facts, one at the encounter grain and one at the condition grain. The dimensions resolve real problems in the feed rather than renaming columns, and the second fact is the shape of the star rather than an addition to it.",
          ],
          bullets: [
            ...
            "Length of stay is an inpatient measure, so the encounter fact publishes it on the 1,728 inpatient rows and null on the other 59,731, where the number would read as a same-day discharge rather than as not applicable. It counts UTC midnights, the discharge date less the admission date on the feed's own clock, and is not the elapsed duration rescaled: the two disagree on 158 of the 1,728",
          ],
        },
        {
          id: "quality",
          title: "Data Quality",
          body: [
            "207 tests run on every build, in CI and locally, with identical results. The interesting ones are the assertions no generic test covers.",
          ],
          bullets: [
        ...
        },
        {
          ...
          title: "Governance",
          body: [
            "The patient dimension applies the HIPAA Safe Harbor rules for names, geography, dates and ages over 89. Names, street address, city, county and coordinates stay in staging. Dates become years, ZIP becomes its first three digits with the seventeen prefixes HHS restricts replaced by 000, and everyone over 89 is aggregated into a single category.",
            "That last part is the one worth reading closely, because capping the age is not enough on its own. The rule removes the elements of dates, the year included, that would reveal an age over 89, so the dimension withholds birth_year and death_year for those 35 patients rather than publishing them beside a capped age. Keeping the years would let one subtraction undo the cap, and joining a birth year to a date on a fact would undo it for every row of that patient. The facts withhold the age outright for those patients rather than capping it, because a cap leaves every exact age below 90 in place, and an exact age beside an exact service date bounds a birth year that a later date turns back into an age. That recovered an age for all 35 of them, to a maximum of 109, until a review caught it.",
            "The claim is scoped to that one model, and to the rules it applies rather than to a Safe Harbor data set. Both facts keep the dates of care on purpose, because a fact that cannot say when something happened is not much of a fact, so the mart layer as a whole is not a Safe Harbor data set. Nor is the patient dimension on its own: its key is the source system's own patient identifier, which Safe Harbor removes. A real release would replace it with a code assigned for that release and keep the crosswalk out of anything it publishes. Doing that here would re-key every patient join, and either give each patient a new key on every build or leave a fresh clone unable to build the marts, so the project keeps the natural key and narrows the claim instead.",
            "The data is synthetic, so this protects nobody. That is the point: the rule is the deliverable, and the difference between the first two tests that enforce it is the lesson. One reads information_schema and fails the build if a forbidden column reappears, but it knows only column names, and it could not have seen the leak above. The other reads the data, and asserts that no birth year the dimension publishes, set beside any date either fact publishes, lands on an age the rule hides. A control that checks names is not a control that checks the rule.", "What that second test does not prove is worth saying plainly, because the scoping is what carries the claim rather than the test. The facts publish exact service dates, so a patient's own span of care can bound an age with no dimension column involved at all: 10 of the 35 have published events more than 89 years apart, and the widest span is 108 years. No test here proves that no combination of published columns recovers a hidden age, and while the facts keep exact dates on purpose, none could. That is why the claim is made for the rules the patient dimension applies and not for the marts.",
          ],
        },
      ],
    },

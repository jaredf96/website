...
export const projects = [
...

  {
    ...
    repoNote: null,
    summary:
      "A dbt project over synthetic electronic health record data: six staged source feeds, a star schema of seven conformed dimensions and two facts at 61,459 encounters and 38,094 conditions, 206 data-quality tests, HIPAA Safe Harbor de-identification enforced by four tests, and CI that builds all of it and publishes the generated documentation on every push. It runs on DuckDB with no account and no credentials: a script fetches the ~565 MB source export once, and the dbt build over it takes under two seconds.",
    context:
    ...
    highlights: [
      ...
      "206 tests: 133 not_null, 21 unique, 19 relationships, 15 accepted_values and 18 singular assertions. Every foreign key in the project resolves with zero orphans",
      ...
      "HIPAA Safe Harbor applied in the patient dimension and enforced by four tests, because each of the first three was not enough. A test that reads information_schema catches a forbidden column, but not a permitted birth year sitting beside a capped age, where one subtraction undoes the cap. The second reads the data, across every date both facts publish. What none of the four proves is stated beside them: the facts keep exact service dates on purpose, so the claim is scoped to the dimension rather than to the marts",
      ...
      "A decision log of 27 entries, each recording what was decided against and what would reopen it",
    ],
    actions: [
      {
        kind: "live",
